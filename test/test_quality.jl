...
@testitem "JET exported API @test_call" tags = [:quality, :jet] begin
    ...
    @test_call target_modules = (MRT,) reconstruct(cart_info, IterativeReconstruction(Tikhonov(0.01)); maxit = 2, verbose = false, threaded = false)
    @test_call target_modules = (MRT,) reconstruct(cart_info; verbose = false, threaded = false)
