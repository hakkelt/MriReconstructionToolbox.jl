...
function draw_superellipsoid!(
        phantom::Array{Float32, 3}, ax_x::AbstractVector, ax_y::AbstractVector, ax_z::AbstractVector,
...
function add_torso_boundary!(phantom, ax_x, ax_y, ax_z)
    ...
    # Shoulders
    draw_superellipsoid!(phantom, ax_x, ax_y, ax_z, 0.0, -0.165, 0.7, 0.8, 0.28, 0.25; ex = (2.5, 2.5, 2.5), intensity = 0.2)
    ...
    # Left arm - mid
    draw_superellipsoid!(phantom, ax_x, ax_y, ax_z, -0.88, -0.165, 0.5, 0.17, 0.17, 0.26; ex = (2.5, 2.5, 2.5), intensity = 0.2)
    ...
    draw_superellipsoid!(phantom, ax_x, ax_y, ax_z, 0.0, 0.28, 0.35, 0.7, 0.43, 0.5; ex = (2.5, 2.5, 2.5), intensity = 0.2)

    # Mid back (mid thoracic region)
    draw_superellipsoid!(phantom, ax_x, ax_y, ax_z, 0.0, 0.28, -0.1, 0.75, 0.47, 0.55; ex = (2.5, 2.5, 2.5), intensity = 0.2)

    # Lower back (lumbar region)
    return draw_superellipsoid!(phantom, ax_x, ax_y, ax_z, 0.0, 0.15, -0.6, 0.78, 0.48, 0.55; ex = (2.5, 2.5, 2.5), intensity = 0.2)
end
...
function add_lungs_axes!(phantom, ax_x, ax_y, ax_z)
    ...
    # Lower lobe
    lung_l_lower_radius = 0.8 * 0.48 * 1.05
    draw_superellipsoid!(phantom, ax_x, ax_y, ax_z, -lung_x_offset, 0.0, -0.2, lung_l_lower_radius, lung_l_lower_radius, 0.45; ex = (2.5, 2.5, 2.5), intensity = 0.09)

    # Diaphragm
    diaphragm_radius = lung_l_lower_radius * 0.8
    draw_superellipsoid!(phantom, ax_x, ax_y, ax_z, -lung_x_offset, 0.0, -0.7, diaphragm_radius, diaphragm_radius, 0.2; ex = (2.5, 2.5, 1.5), intensity = 0.2)
    ...
    draw_superellipsoid!(phantom, ax_x, ax_y, ax_z, lung_x_offset, 0.0, -0.2, lung_l_lower_radius, lung_l_lower_radius, 0.45; ex = (2.5, 2.5, 2.5), intensity = 0.09)
    # Diaphragm
    return draw_superellipsoid!(phantom, ax_x, ax_y, ax_z, lung_x_offset, 0.0, -0.7, diaphragm_radius, diaphragm_radius, 0.2; ex = (2.5, 2.5, 1.5), intensity = 0.2)
...
function add_heart_axes!(phantom, ax_x, ax_y, ax_z)
    # Heart main body
    # Upper heart (base)
    draw_superellipsoid!(phantom, ax_x, ax_y, ax_z, 0.05, 0.0, -0.1, 0.25, 0.25, 0.2; ex = (2.0, 2.0, 2.0), intensity = 0.6)
    ...
    # Right ventricle
    draw_superellipsoid!(phantom, ax_x, ax_y, ax_z, 0.1, 0.0, 0.0, 0.15, 0.15, 0.14; ex = (2.0, 2.0, 2.0), intensity = 0.65)
    draw_superellipsoid!(phantom, ax_x, ax_y, ax_z, 0.1, 0.0, 0.12, 0.12, 0.12, 0.13; ex = (3.0, 3.0, 2.0), intensity = 0.65)

    # Left atrium
    draw_superellipsoid!(phantom, ax_x, ax_y, ax_z, 0.03, 0.05, -0.2, 0.12, 0.12, 0.15; ex = (2.2, 2.2, 2.2), intensity = 0.68)

    # Right atrium
    return draw_superellipsoid!(phantom, ax_x, ax_y, ax_z, 0.03, 0.07, -0.2, 0.12, 0.12, 0.15; ex = (2.2, 2.2, 2.2), intensity = 0.63)
end
...
function add_vessels_axes!(phantom, ax_x, ax_y, ax_z)
    function draw_vessel!(x_center, y_center, z_center, radius_xy, height_z, intensity, n)
        return draw_superellipsoid!(phantom, ax_x, ax_y, ax_z, x_center, y_center, z_center, radius_xy, radius_xy, height_z; ex = (n, n, n), intensity = intensity)
    end

    # Aorta (ascending)
    for (z_center, half_height) in [(0.7, 0.08), (0.55, 0.12), (0.4, 0.12), (0.25, 0.12), (0.1, 0.12)]
        draw_vessel!(-0.02, -0.05, z_center, 0.06, half_height, 0.8, 2.5)
    end

    # Pulmonary artery
    for (z_center, half_height) in [(0.6, 0.08), (0.47, 0.1), (0.32, 0.12), (0.17, 0.12), (0.05, 0.1)]
        draw_vessel!(-0.05, -0.05, z_center, 0.05, half_height, 0.75, 2.5)
    end

    # Superior vena cava
    for (z_center, half_height) in [(0.85, 0.08), (0.72, 0.1), (0.58, 0.12), (0.45, 0.12), (0.35, 0.08)]
...
function add_ribs_axes!(phantom, ax_x, ax_y, ax_z)
    ...
    create_rib_curve(0.0, num_segments, 0.8, 0.62, 1.0)
    create_rib_curve(-0.15, num_segments, 0.8, 0.62, 0.9)

    create_rib_curve(-0.3, num_segments, 0.78, 0.58, 0.75)
    create_rib_curve(-0.45, num_segments, 0.75, 0.57, 0.6)
    return create_rib_curve(-0.6, num_segments, 0.65, 0.53, 0.5)
end
...
function add_liver_axes!(phantom, ax_x, ax_y, ax_z)
    ...
    draw_superellipsoid!(phantom, ax_x, ax_y, ax_z, 0.25, 0.15, -0.8, 0.35, 0.3, 0.3; ex = (2.5, 2.5, 2.5), intensity = 0.45)

    # Left lobe
    return draw_superellipsoid!(phantom, ax_x, ax_y, ax_z, -0.05, 0.12, -0.75, 0.2, 0.25, 0.25; ex = (2.5, 2.5, 2.5), intensity = 0.43)
end
...
function add_stomach_axes!(phantom, ax_x, ax_y, ax_z)
    ...
    # Fundus
    draw_superellipsoid!(phantom, ax_x, ax_y, ax_z, -0.25, 0.05, -0.7, 0.3, 0.18, 0.2; ex = (2.5, 2.5, 2.5), intensity = 0.38)

    # Body
    return draw_superellipsoid!(phantom, ax_x, ax_y, ax_z, -0.15, 0.08, -0.8, 0.16, 0.16, 0.22; ex = (2.5, 2.5, 2.5), intensity = 0.36)
end
