...
julia> img = rand(ComplexF32, 64, 64, 8);

julia> F = get_fourier_operator(img, false)
ℱ*±  ℂ^(64, 64, 8) -> ℂ^(64, 64, 8)

julia> F * img ≈ fftshift(fft(img, (1,2)), (1,2)) # both kspace dims are shifted back by default to center
true

julia> F = get_fourier_operator(img, false; shifted_kspace_dims=1) # x-dim is already pre-shifted
Π  ℂ^(64, 64, 8) -> ℂ^(64, 64, 8)

julia> F * img ≈ fftshift(fft(img, (1,2)), (2,)) # only y-dim is shifted back
true

julia> F = get_fourier_operator(img, false; shifted_image_dims=(1,2)) # do default kspace shifts, but shift image dims too
Π  ℂ^(64, 64, 8) -> ℂ^(64, 64, 8)

julia> F * img ≈ fftshift(fft(fftshift(img, (1,2)), (1,2)), (1,2))
true

julia> img_3d = rand(ComplexF32, 64, 64, 32, 8);

julia> F_3d = get_fourier_operator(img_3d, true)
ℱ*±  ℂ^(64, 64, 32, 8) -> ℂ^(64, 64, 32, 8) 

julia> F_3d * img_3d ≈ fftshift(fft(img_3d, (1,2,3)), (1,2,3))
true

julia> using NamedDims

julia> ksp = NamedDimsArray{(:kx, :ky, :kz, :coil)}(rand(ComplexF32, 64, 64, 32, 8));

julia> F = get_fourier_operator(ksp)
ℱ*±  ℂ^(64, 64, 32, 8) -> ℂ^(64, 64, 32, 8)

julia> dimnames(F' * ksp)
(:x, :y, :z, :coil)

julia> acq_data = AcquisitionInfo(ksp, shifted_kspace_dims=(1,2), shifted_image_dims=:x)
AcquisitionInfo:
  - kspace data = ComplexF32<kx: 64, ky: 64, kz: 32, coil: 8>
  - encoding = 3D
  - image size = 64×64×32
  - shifted kspace dims = (1,2)
  - shifted image dims = x

julia> F = get_fourier_operator(acq_data)
ℱ*±  ℂ^(64, 64, 32, 8) -> ℂ^(64, 64, 32, 8)
