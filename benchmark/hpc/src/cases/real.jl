    ...
    for (name, reg) in (
        ...
        push!(
            ...
            (
                ...
                run = () -> reconstruct(
                    acq_us, IterativeReconstruction(regularization = reg; maxit = 30, reltol = 1.0e-5); verbosity = Silent()
                ),
            ),
        )
    end
    ...
    return _try_real("Real 3D") do
        ...
        NamedTuple[
            (
                ...
                run = () -> reconstruct(
                    acq_full,
                    IterativeReconstruction(regularization = (), algorithm = MriReconstructionToolbox.CGNR(maxit = 10, tol = 1.0e-14); maxit = 10, reltol = 1.0e-14); verbosity = Silent()
                ),
            ),
            (
                category = "Real 3D", method = "Total Variation (30 it)", reference = ref,
                run = () -> reconstruct(
                    acq_us, IterativeReconstruction(regularization = TotalVariation2D(0.01); maxit = 30, reltol = 1.0e-5); verbosity = Silent()
                ),
            ),
        ]
    end
        ...
        for (name, reg) in (
            ...
            push!(
                ...
                (
                    ...
                    run = () -> reconstruct(acq, IterativeReconstruction(regularization = reg; maxit = 20, reltol = 1.0e-4); verbosity = Silent()),
                ),
            )
        end
        cases
