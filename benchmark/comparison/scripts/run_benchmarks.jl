# benchmark/comparison/scripts/run_benchmarks.jl
#
...
# BART_USE_FFTW_WISDOM=1 was measured to *hurt* here (benchmark/hpc/scripts/bart_wisdom_probe.sh):
...
function time_reconstruction(f; num_runs = 3, is_bart = false)
    ...
    t_med = median(times)
    if is_bart
        t_min = max(1.0e-5, t_min - bart_startup_time)
        t_med = max(1.0e-5, t_med - bart_startup_time)
    end
    return t_min, t_med, res
end

# MRT baseline from benchmark/hpc/. The MRT-only reconstruction timings are owned by
# benchmark/hpc/scripts/recon_bench.jl and measured there on the same phantoms; if that JSON is
...
#   julia --project=benchmark/hpc -t N benchmark/hpc/scripts/recon_bench.jl --threads=N [--use-mkl]
...
# Time an MRT reconstruction, or return the benchmark/hpc/ baseline time if available. Same
