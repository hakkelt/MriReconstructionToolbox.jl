...
# (case, method, threads, backend, environment variant) measured under both, the latest result of
# each side is compared: B/A of the minimum time, each side's spread (median / minimum), and
...
# Latest ok row per key for one side.
function side(ref)
    best = Dict{Tuple, Tuple{String, Dict, Dict}}()
    for d in ResultsStore.load_run_files(ResultsStore.HARNESS_RESULTS_DIR)
        ...
        for r in get(d, "benchmarks", [])
            ...
            ts = get(d, "ts", "")
            (!haskey(best, key) || best[key][1] < ts) && (best[key] = (ts, d, r))
        end
    end
    return best
end
...
isempty(SA) && error("no harness results for $A")
isempty(SB) && error("no harness results for $B")
...
for (label, S) in ((A, SA), (B, SB))
    ...
    length(commits) > 1 && @warn "$label matches results from several commits; the latest per configuration is used" commits
end
