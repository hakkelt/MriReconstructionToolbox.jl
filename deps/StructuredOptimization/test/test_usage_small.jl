...
# `ls`'s normal-op path (auto-selected here since a single-variable `A_tall*x - b_tall` has
# a non-identity linear operator) hands its already-built `AᴴA` to ADMM instead of letting
# ADMM build a second one (`prepare` emits the assumption's `AHA` key). The cached operator
# must be the *linear* normal operator: `prepare` strips the `-b` displacement from `A`, so
# it must strip it here too.
let
    x_nop = Variable(4)
    p_nop = problem(ls(A_tall * x_nop - b_tall) + 1.0e-2 * norm(x_nop, 1))
    ...
    # not be forwarded.
    x_scaled = Variable(4)
    p_scaled = problem(2.0 * ls(A_tall * x_scaled - b_tall) + 1.0e-2 * norm(x_scaled, 1))
